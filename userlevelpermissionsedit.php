...
loadjs.ready("head", function() {
	...
	fuserlevelpermissionsedit.lists["x_userlevelid"] = <?php echo $userlevelpermissions_edit->userlevelid->Lookup->toClientList($userlevelpermissions_edit) ?>;
	fuserlevelpermissionsedit.lists["x_userlevelid"].options = <?php echo JsonEncode($userlevelpermissions_edit->userlevelid->lookupOptions()) ?>;
...
<div class="input-group">
	<select class="custom-select ew-custom-select" data-table="userlevelpermissions" data-field="x_userlevelid" data-value-separator="<?php echo $userlevelpermissions_edit->userlevelid->displayValueSeparatorAttribute() ?>" id="x_userlevelid" name="x_userlevelid"<?php echo $userlevelpermissions_edit->userlevelid->editAttributes() ?>>
			<?php echo $userlevelpermissions_edit->userlevelid->selectOptionListHtml("x_userlevelid") ?>
		</select>
</div>
<?php echo $userlevelpermissions_edit->userlevelid->Lookup->getParamTag($userlevelpermissions_edit, "p_x_userlevelid") ?>
...
<?php if ($userlevelpermissions_edit->permission->Visible) { // permission ?>
	<div id="r_permission" class="form-group row">
		<label id="elh_userlevelpermissions_permission" class="<?php echo $userlevelpermissions_edit->LeftColumnClass ?>"><?php echo $userlevelpermissions_edit->permission->caption() ?><?php echo $userlevelpermissions_edit->permission->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<div id="tp_x_permission" class="ew-template"><input type="checkbox" class="custom-control-input" data-table="userlevelpermissions" data-field="x_permission" data-value-separator="<?php echo $userlevelpermissions_edit->permission->displayValueSeparatorAttribute() ?>" name="x_permission[]" id="x_permission[]" value="{value}"<?php echo $userlevelpermissions_edit->permission->editAttributes() ?>></div>
<div id="dsl_x_permission" data-repeatcolumn="5" class="ew-item-list d-none"><div>
<?php echo $userlevelpermissions_edit->permission->checkBoxListHtml(FALSE, "x_permission[]") ?>
</div></div>
</span>
