...
/**
 * Table class for basic_salaries
 */
class basic_salaries extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'basic_salaries';
		$this->TableName = 'basic_salaries';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`basic_salaries`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('basic_salaries', 'basic_salaries', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Name
		$this->Name = new DbField('basic_salaries', 'basic_salaries', 'x_Name', 'Name', '`Name`', '`Name`', 200, 201, -1, FALSE, '`Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// BasicSalary
		$this->BasicSalary = new DbField('basic_salaries', 'basic_salaries', 'x_BasicSalary', 'BasicSalary', '`BasicSalary`', '`BasicSalary`', 5, 23, -1, FALSE, '`BasicSalary`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`basic_salaries`";
	}
	...
	{
		global $Language;
		if ($pageName == "basic_salariesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "basic_salariesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "basic_salariesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("basic_salariesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("basic_salariesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "basic_salariesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "basic_salariesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("basic_salariesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("basic_salariesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("basic_salariesdelete.php", $this->getUrlParm());
	}
