...
/**
 * Table class for gross_income
 */
class gross_income extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'gross_income';
		$this->TableName = 'gross_income';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`gross_income`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('gross_income', 'gross_income', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Gross
		$this->Gross = new DbField('gross_income', 'gross_income', 'x_Gross', 'Gross', '`Gross`', '`Gross`', 5, 23, -1, FALSE, '`Gross`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`gross_income`";
	}
	...
	{
		global $Language;
		if ($pageName == "gross_incomeview.php")
			return $Language->phrase("View");
		elseif ($pageName == "gross_incomeedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "gross_incomeadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("gross_incomeview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("gross_incomeview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "gross_incomeadd.php?" . $this->getUrlParm($parm);
		else
			$url = "gross_incomeadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("gross_incomeedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("gross_incomeadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("gross_incomedelete.php", $this->getUrlParm());
	}
