...
/**
 * Table class for debit
 */
class debit extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// PayrollPeriod
		$this->PayrollPeriod = new DbField('debit', 'debit', 'x_PayrollPeriod', 'PayrollPeriod', '`PayrollPeriod`', '`PayrollPeriod`', 3, 11, -1, FALSE, '`PayrollPeriod`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DeductionCode
		$this->DeductionCode = new DbField('debit', 'debit', 'x_DeductionCode', 'DeductionCode', '`DeductionCode`', '`DeductionCode`', 200, 15, -1, FALSE, '`DeductionCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DeductionName
		$this->DeductionName = new DbField('debit', 'debit', 'x_DeductionName', 'DeductionName', '`DeductionName`', '`DeductionName`', 200, 255, -1, FALSE, '`DeductionName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// TotalDeductions
		$this->TotalDeductions = new DbField('debit', 'debit', 'x_TotalDeductions', 'TotalDeductions', '`TotalDeductions`', '`TotalDeductions`', 5, 23, -1, FALSE, '`TotalDeductions`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`debit`";
	}
	...
	{
		global $Language;
		if ($pageName == "debitview.php")
			return $Language->phrase("View");
		elseif ($pageName == "debitedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "debitadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("debitview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("debitview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "debitadd.php?" . $this->getUrlParm($parm);
		else
			$url = "debitadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("debitedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("debitadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("debitdelete.php", $this->getUrlParm());
	}
