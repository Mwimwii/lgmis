...
/**
 * Table class for employee_obligation
 */
class employee_obligation extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'employee_obligation';
		$this->TableName = 'employee_obligation';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`employee_obligation`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('employee_obligation', 'employee_obligation', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// PaidPosition
		$this->PaidPosition = new DbField('employee_obligation', 'employee_obligation', 'x_PaidPosition', 'PaidPosition', '`PaidPosition`', '`PaidPosition`', 3, 11, -1, FALSE, '`PaidPosition`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PaidPosition->Nullable = FALSE; // NOT NULL field
		$this->PaidPosition->Required = TRUE; // Required field
		...
		// PayrollDate
		$this->PayrollDate = new DbField('employee_obligation', 'employee_obligation', 'x_PayrollDate', 'PayrollDate', '`PayrollDate`', CastDateFieldForLike("`PayrollDate`", 0, "DB"), 135, 19, 0, FALSE, '`PayrollDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// PayrollPeriod
		$this->PayrollPeriod = new DbField('employee_obligation', 'employee_obligation', 'x_PayrollPeriod', 'PayrollPeriod', '`PayrollPeriod`', '`PayrollPeriod`', 3, 11, -1, FALSE, '`PayrollPeriod`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// StartDate
		$this->StartDate = new DbField('employee_obligation', 'employee_obligation', 'x_StartDate', 'StartDate', '`StartDate`', CastDateFieldForLike("`StartDate`", 0, "DB"), 133, 10, 0, FALSE, '`StartDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Enddate
		$this->Enddate = new DbField('employee_obligation', 'employee_obligation', 'x_Enddate', 'Enddate', '`Enddate`', CastDateFieldForLike("`Enddate`", 0, "DB"), 133, 10, 0, FALSE, '`Enddate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ObligationCode
		$this->ObligationCode = new DbField('employee_obligation', 'employee_obligation', 'x_ObligationCode', 'ObligationCode', '`ObligationCode`', '`ObligationCode`', 200, 15, -1, FALSE, '`ObligationCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ObligationCode->IsPrimaryKey = TRUE; // Primary key field
		$this->ObligationCode->Nullable = FALSE; // NOT NULL field
		$this->ObligationCode->Required = TRUE; // Required field
		$this->ObligationCode->Sortable = TRUE; // Allow sort
		$this->fields['ObligationCode'] = &$this->ObligationCode;

		// ObligationAmount
		$this->ObligationAmount = new DbField('employee_obligation', 'employee_obligation', 'x_ObligationAmount', 'ObligationAmount', '`ObligationAmount`', '`ObligationAmount`', 5, 22, -1, FALSE, '`ObligationAmount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ObligationAmount->Nullable = FALSE; // NOT NULL field
		$this->ObligationAmount->Sortable = TRUE; // Allow sort
		$this->ObligationAmount->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['ObligationAmount'] = &$this->ObligationAmount;

		// Remarks
		$this->Remarks = new DbField('employee_obligation', 'employee_obligation', 'x_Remarks', 'Remarks', '`Remarks`', '`Remarks`', 200, 255, -1, FALSE, '`Remarks`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`employee_obligation`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			...
			if (array_key_exists('ObligationCode', $rs))
				AddFilter($where, QuotedName('ObligationCode', $this->Dbid) . '=' . QuotedValue($rs['ObligationCode'], $this->ObligationCode->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->ObligationCode->DbValue = $row['ObligationCode'];
		$this->ObligationAmount->DbValue = $row['ObligationAmount'];
	...
	{
		return "`EmployeeID` = @EmployeeID@ AND `PayrollPeriod` = @PayrollPeriod@ AND `ObligationCode` = '@ObligationCode@'";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('ObligationCode', $row) ? $row['ObligationCode'] : NULL;
		else
			$val = $this->ObligationCode->OldValue !== NULL ? $this->ObligationCode->OldValue : $this->ObligationCode->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@ObligationCode@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "employee_obligationlist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "employee_obligationedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "employee_obligationadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("employee_obligationview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("employee_obligationview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "employee_obligationadd.php?" . $this->getUrlParm($parm);
		else
			$url = "employee_obligationadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("employee_obligationedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("employee_obligationadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("employee_obligationdelete.php", $this->getUrlParm());
	}
	...
	public function keyToJson($htmlEncode = FALSE)
	{
		...
		$json .= ",ObligationCode:" . JsonEncode($this->ObligationCode->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->ObligationCode->CurrentValue != NULL) {
			$url .= "&ObligationCode=" . urlencode($this->ObligationCode->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		return $url;
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			elseif (IsApi() && Route(3) !== NULL)
				...
				$arKeys = NULL; // Do not setup
			if (Param("ObligationCode") !== NULL)
				$arKey[] = Param("ObligationCode");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->ObligationCode->CurrentValue = $key[2];
			else
				$this->ObligationCode->OldValue = $key[2];
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->ObligationCode->setDbValue($rs->fields('ObligationCode'));
		$this->ObligationAmount->setDbValue($rs->fields('ObligationAmount'));
	...
	public function renderEditRow()
	{
		...
		$this->ObligationCode->EditValue = $this->ObligationCode->CurrentValue;
		...
		$this->ObligationAmount->EditValue = $this->ObligationAmount->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->ObligationCode);
					$doc->exportCaption($this->ObligationAmount);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->ObligationCode);
						$doc->exportField($this->ObligationAmount);
