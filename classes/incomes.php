...
/**
 * Table class for incomes
 */
class incomes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'incomes';
		$this->TableName = 'incomes';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`incomes`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('incomes', 'incomes', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// IncomeCode
		$this->IncomeCode = new DbField('incomes', 'incomes', 'x_IncomeCode', 'IncomeCode', '`IncomeCode`', '`IncomeCode`', 200, 15, -1, FALSE, '`IncomeCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// IncomeName
		$this->IncomeName = new DbField('incomes', 'incomes', 'x_IncomeName', 'IncomeName', '`IncomeName`', '`IncomeName`', 200, 255, -1, FALSE, '`IncomeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Income
		$this->Income = new DbField('incomes', 'incomes', 'x_Income', 'Income', '`Income`', '`Income`', 5, 22, -1, FALSE, '`Income`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`incomes`";
	}
	...
	{
		return "`EmployeeID` = @EmployeeID@ AND `IncomeCode` = '@IncomeCode@'";
	}
	...
	{
		global $Language;
		if ($pageName == "incomesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "incomesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "incomesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("incomesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("incomesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "incomesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "incomesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("incomesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("incomesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("incomesdelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->IncomeCode->CurrentValue = $key[1];
			else
				$this->IncomeCode->OldValue = $key[1];
