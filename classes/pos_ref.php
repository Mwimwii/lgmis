...
/**
 * Table class for pos_ref
 */
class pos_ref extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'pos_ref';
		$this->TableName = 'pos_ref';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`pos_ref`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// ProvinceCode
		$this->ProvinceCode = new DbField('pos_ref', 'pos_ref', 'x_ProvinceCode', 'ProvinceCode', '`ProvinceCode`', '`ProvinceCode`', 16, 3, -1, FALSE, '`ProvinceCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// LAcode
		$this->LAcode = new DbField('pos_ref', 'pos_ref', 'x_LAcode', 'LAcode', '`LAcode`', '`LAcode`', 3, 4, -1, FALSE, '`LAcode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// PositionName
		$this->PositionName = new DbField('pos_ref', 'pos_ref', 'x_PositionName', 'PositionName', '`PositionName`', '`PositionName`', 200, 255, -1, FALSE, '`PositionName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PositionName->Sortable = TRUE; // Allow sort
		$this->fields['PositionName'] = &$this->PositionName;

		// SalaryScale
		$this->SalaryScale = new DbField('pos_ref', 'pos_ref', 'x_SalaryScale', 'SalaryScale', '`SalaryScale`', '`SalaryScale`', 200, 13, -1, FALSE, '`SalaryScale`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->SalaryScale->Sortable = TRUE; // Allow sort
		$this->fields['SalaryScale'] = &$this->SalaryScale;

		// Sectionname
		$this->Sectionname = new DbField('pos_ref', 'pos_ref', 'x_Sectionname', 'Sectionname', '`Sectionname`', '`Sectionname`', 200, 255, -1, FALSE, '`Sectionname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Sectionname->Sortable = TRUE; // Allow sort
		$this->fields['Sectionname'] = &$this->Sectionname;

		// CouncilType
		$this->CouncilType = new DbField('pos_ref', 'pos_ref', 'x_CouncilType', 'CouncilType', '`CouncilType`', '`CouncilType`', 16, 3, -1, FALSE, '`CouncilType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`pos_ref`";
	}
	...
	{
		global $Language;
		if ($pageName == "pos_refview.php")
			return $Language->phrase("View");
		elseif ($pageName == "pos_refedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "pos_refadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("pos_refview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("pos_refview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "pos_refadd.php?" . $this->getUrlParm($parm);
		else
			$url = "pos_refadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("pos_refedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("pos_refadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("pos_refdelete.php", $this->getUrlParm());
	}
