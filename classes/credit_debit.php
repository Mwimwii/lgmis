...
/**
 * Table class for credit_debit
 */
class credit_debit extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'credit_debit';
		$this->TableName = 'credit_debit';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`credit_debit`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// CREDIT
		$this->CREDIT = new DbField('credit_debit', 'credit_debit', 'x_CREDIT', 'CREDIT', '`CREDIT`', '`CREDIT`', 200, 6, -1, FALSE, '`CREDIT`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// IncomeName
		$this->IncomeName = new DbField('credit_debit', 'credit_debit', 'x_IncomeName', 'IncomeName', '`IncomeName`', '`IncomeName`', 200, 255, -1, FALSE, '`IncomeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// TotalIncomes
		$this->TotalIncomes = new DbField('credit_debit', 'credit_debit', 'x_TotalIncomes', 'TotalIncomes', '`TotalIncomes`', '`TotalIncomes`', 5, 23, -1, FALSE, '`TotalIncomes`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->TotalIncomes->Sortable = TRUE; // Allow sort
		$this->TotalIncomes->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['TotalIncomes'] = &$this->TotalIncomes;

		// DEBIT
		$this->DEBIT = new DbField('credit_debit', 'credit_debit', 'x_DEBIT', 'DEBIT', '`DEBIT`', '`DEBIT`', 201, 65530, -1, FALSE, '`DEBIT`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DeductionName
		$this->DeductionName = new DbField('credit_debit', 'credit_debit', 'x_DeductionName', 'DeductionName', '`DeductionName`', '`DeductionName`', 201, 65530, -1, FALSE, '`DeductionName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// TotalDeductions
		$this->TotalDeductions = new DbField('credit_debit', 'credit_debit', 'x_TotalDeductions', 'TotalDeductions', '`TotalDeductions`', '`TotalDeductions`', 201, 65530, -1, FALSE, '`TotalDeductions`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`credit_debit`";
	}
	...
	{
		global $Language;
		if ($pageName == "credit_debitview.php")
			return $Language->phrase("View");
		elseif ($pageName == "credit_debitedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "credit_debitadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("credit_debitview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("credit_debitview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "credit_debitadd.php?" . $this->getUrlParm($parm);
		else
			$url = "credit_debitadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("credit_debitedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("credit_debitadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("credit_debitdelete.php", $this->getUrlParm());
	}
	...
	public function renderEditRow()
	{
		...
		if (strval($this->TotalIncomes->EditValue) != "" && is_numeric($this->TotalIncomes->EditValue))
			$this->TotalIncomes->EditValue = FormatNumber($this->TotalIncomes->EditValue, -2, -2, -2, -2);
		...
		if (!$this->TotalDeductions->Raw)
			$this->TotalDeductions->CurrentValue = HtmlDecode($this->TotalDeductions->CurrentValue);
