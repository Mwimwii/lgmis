...
/**
 * Table class for payroll_period
 */
class payroll_period extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'payroll_period';
		$this->TableName = 'payroll_period';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`payroll_period`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// PeriodCode
		$this->PeriodCode = new DbField('payroll_period', 'payroll_period', 'x_PeriodCode', 'PeriodCode', '`PeriodCode`', '`PeriodCode`', 3, 11, -1, FALSE, '`PeriodCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// FiscalYear
		$this->FiscalYear = new DbField('payroll_period', 'payroll_period', 'x_FiscalYear', 'FiscalYear', '`FiscalYear`', '`FiscalYear`', 18, 4, -1, FALSE, '`FiscalYear`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->FiscalYear->IsForeignKey = TRUE; // Foreign key field
		...
		$this->FiscalYear->Lookup = new Lookup('FiscalYear', 'years', FALSE, 'Year', ["Year","","",""], [], [], [], [], [], [], '', '');
		$this->FiscalYear->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['FiscalYear'] = &$this->FiscalYear;

		// RunMonth
		$this->RunMonth = new DbField('payroll_period', 'payroll_period', 'x_RunMonth', 'RunMonth', '`RunMonth`', '`RunMonth`', 16, 2, -1, FALSE, '`RunMonth`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// RunDescription
		$this->RunDescription = new DbField('payroll_period', 'payroll_period', 'x_RunDescription', 'RunDescription', '`RunDescription`', '`RunDescription`', 200, 255, -1, FALSE, '`RunDescription`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->RunDescription->Sortable = TRUE; // Allow sort
		$this->fields['RunDescription'] = &$this->RunDescription;

		// CurrentPeriod
		$this->CurrentPeriod = new DbField('payroll_period', 'payroll_period', 'x_CurrentPeriod', 'CurrentPeriod', '`CurrentPeriod`', '`CurrentPeriod`', 16, 1, -1, FALSE, '`CurrentPeriod`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		...
		$this->CurrentPeriod->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "employee_employer_schedule_view") {
			$detailUrl = $GLOBALS["employee_employer_schedule_view"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_PeriodCode=" . urlencode($this->PeriodCode->CurrentValue);
		}
		if ($this->getCurrentDetailTable() == "obligation_schedule_view") {
			$detailUrl = $GLOBALS["obligation_schedule_view"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_PeriodCode=" . urlencode($this->PeriodCode->CurrentValue);
		}
		if ($this->getCurrentDetailTable() == "deduction_schedule_view") {
			$detailUrl = $GLOBALS["deduction_schedule_view"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_PeriodCode=" . urlencode($this->PeriodCode->CurrentValue);
		}
		if ($this->getCurrentDetailTable() == "income_schedule_view") {
			$detailUrl = $GLOBALS["income_schedule_view"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_PeriodCode=" . urlencode($this->PeriodCode->CurrentValue);
		}
		if ($this->getCurrentDetailTable() == "monthly_run") {
			$detailUrl = $GLOBALS["monthly_run"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_PeriodCode=" . urlencode($this->PeriodCode->CurrentValue);
			$detailUrl .= "&fk_FiscalYear=" . urlencode($this->FiscalYear->CurrentValue);
			$detailUrl .= "&fk_RunMonth=" . urlencode($this->RunMonth->CurrentValue);
		}
		if ($this->getCurrentDetailTable() == "payroll_summary_view") {
			$detailUrl = $GLOBALS["payroll_summary_view"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_PeriodCode=" . urlencode($this->PeriodCode->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "payroll_periodlist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`payroll_period`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "payroll_periodedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "payroll_periodadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("payroll_periodview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("payroll_periodview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "payroll_periodadd.php?" . $this->getUrlParm($parm);
		else
			$url = "payroll_periodadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("payroll_periodedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("payroll_periodedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("payroll_periodadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("payroll_periodadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("payroll_perioddelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->PeriodCode->CurrentValue = $key;
			else
				$this->PeriodCode->OldValue = $key;
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		$cur = $rsnew["CurrentPeriod"];
		$other = executeScalar("select CurrentPeriod from payroll_period where CurrentPeriod = 1");
		if($cur == 1 && $other == 1) {
		$this->CancelMessage =  "Only one current period should be open. Please correct." ;
		return FALSE;
		}
		return TRUE;
	}
