...
class category extends DbTable
{
	...
	public function __construct()
	{
		...
		// CategoryCode
		$this->CategoryCode = new DbField('category', 'category', 'x_CategoryCode', 'CategoryCode', '`CategoryCode`', '`CategoryCode`', 2, 5, -1, FALSE, '`CategoryCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CategoryCode->Nullable = FALSE; // NOT NULL field
		$this->CategoryCode->Sortable = TRUE; // Allow sort
		$this->CategoryCode->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['CategoryCode'] = &$this->CategoryCode;

		// CategoryName
		$this->CategoryName = new DbField('category', 'category', 'x_CategoryName', 'CategoryName', '`CategoryName`', '`CategoryName`', 200, 255, -1, FALSE, '`CategoryName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// CategoryDesc
		$this->CategoryDesc = new DbField('category', 'category', 'x_CategoryDesc', 'CategoryDesc', '`CategoryDesc`', '`CategoryDesc`', 200, 255, -1, FALSE, '`CategoryDesc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CategoryDesc->Sortable = TRUE; // Allow sort
	...
	public function renderEditRow()
	{
		...
		$this->CategoryDesc->EditValue = $this->CategoryDesc->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->CategoryDesc);
				} else {
					$doc->exportCaption($this->CategoryCode);
					$doc->exportCaption($this->CategoryName);
					$doc->exportCaption($this->CategoryDesc);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->CategoryDesc);
					} else {
						$doc->exportField($this->CategoryCode);
						$doc->exportField($this->CategoryName);
						$doc->exportField($this->CategoryDesc);
