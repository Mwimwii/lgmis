...
/**
 * Table class for house_rent
 */
class house_rent extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'house_rent';
		$this->TableName = 'house_rent';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`house_rent`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('house_rent', 'house_rent', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// FirstName
		$this->FirstName = new DbField('house_rent', 'house_rent', 'x_FirstName', 'FirstName', '`FirstName`', '`FirstName`', 200, 100, -1, FALSE, '`FirstName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// MaidenName
		$this->MaidenName = new DbField('house_rent', 'house_rent', 'x_MaidenName', 'MaidenName', '`MaidenName`', '`MaidenName`', 200, 100, -1, FALSE, '`MaidenName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->MaidenName->Sortable = TRUE; // Allow sort
		$this->fields['MaidenName'] = &$this->MaidenName;

		// Surname
		$this->Surname = new DbField('house_rent', 'house_rent', 'x_Surname', 'Surname', '`Surname`', '`Surname`', 200, 100, -1, FALSE, '`Surname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Period
		$this->Period = new DbField('house_rent', 'house_rent', 'x_Period', 'Period', '`Period`', '`Period`', 200, 25, -1, FALSE, '`Period`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DeductionCode
		$this->DeductionCode = new DbField('house_rent', 'house_rent', 'x_DeductionCode', 'DeductionCode', '`DeductionCode`', '`DeductionCode`', 200, 15, -1, FALSE, '`DeductionCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// House Rent
		$this->House_Rent = new DbField('house_rent', 'house_rent', 'x_House_Rent', 'House Rent', '`House Rent`', '`House Rent`', 5, 22, -1, FALSE, '`House Rent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->House_Rent->Nullable = FALSE; // NOT NULL field
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`house_rent`";
	}
	...
	{
		return "`EmployeeID` = @EmployeeID@ AND `DeductionCode` = '@DeductionCode@'";
	}
	...
	{
		global $Language;
		if ($pageName == "house_rentview.php")
			return $Language->phrase("View");
		elseif ($pageName == "house_rentedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "house_rentadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("house_rentview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("house_rentview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "house_rentadd.php?" . $this->getUrlParm($parm);
		else
			$url = "house_rentadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("house_rentedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("house_rentadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("house_rentdelete.php", $this->getUrlParm());
	}
	...
	public function keyToJson($htmlEncode = FALSE)
	{
		...
		$json .= ",DeductionCode:" . JsonEncode($this->DeductionCode->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->DeductionCode->CurrentValue = $key[1];
			else
				$this->DeductionCode->OldValue = $key[1];
