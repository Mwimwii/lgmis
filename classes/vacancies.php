...
/**
 * Table class for vacancies
 */
class vacancies extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vacancies';
		$this->TableName = 'vacancies';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vacancies`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// PROVINCE
		$this->PROVINCE = new DbField('vacancies', 'vacancies', 'x_PROVINCE', 'PROVINCE', '`PROVINCE`', '`PROVINCE`', 200, 40, -1, FALSE, '`PROVINCE`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PROVINCE->Sortable = TRUE; // Allow sort
		$this->fields['PROVINCE'] = &$this->PROVINCE;

		// LOCAL AUTHORITY
		$this->LOCAL_AUTHORITY = new DbField('vacancies', 'vacancies', 'x_LOCAL_AUTHORITY', 'LOCAL AUTHORITY', '`LOCAL AUTHORITY`', '`LOCAL AUTHORITY`', 200, 40, -1, FALSE, '`LOCAL AUTHORITY`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->LOCAL_AUTHORITY->Sortable = TRUE; // Allow sort
		$this->fields['LOCAL AUTHORITY'] = &$this->LOCAL_AUTHORITY;

		// DEPARTMENT
		$this->DEPARTMENT = new DbField('vacancies', 'vacancies', 'x_DEPARTMENT', 'DEPARTMENT', '`DEPARTMENT`', '`DEPARTMENT`', 200, 255, -1, FALSE, '`DEPARTMENT`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->DEPARTMENT->Sortable = TRUE; // Allow sort
		$this->fields['DEPARTMENT'] = &$this->DEPARTMENT;

		// SECTION
		$this->SECTION = new DbField('vacancies', 'vacancies', 'x_SECTION', 'SECTION', '`SECTION`', '`SECTION`', 200, 255, -1, FALSE, '`SECTION`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->SECTION->Sortable = TRUE; // Allow sort
		$this->fields['SECTION'] = &$this->SECTION;

		// POSITION
		$this->POSITION = new DbField('vacancies', 'vacancies', 'x_POSITION', 'POSITION', '`POSITION`', '`POSITION`', 200, 255, -1, FALSE, '`POSITION`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->POSITION->Sortable = TRUE; // Allow sort
		$this->fields['POSITION'] = &$this->POSITION;

		// SALARY SCALE
		$this->SALARY_SCALE = new DbField('vacancies', 'vacancies', 'x_SALARY_SCALE', 'SALARY SCALE', '`SALARY SCALE`', '`SALARY SCALE`', 200, 10, -1, FALSE, '`SALARY SCALE`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->SALARY_SCALE->Nullable = FALSE; // NOT NULL field
		$this->SALARY_SCALE->Required = TRUE; // Required field
		$this->SALARY_SCALE->Sortable = TRUE; // Allow sort
		$this->fields['SALARY SCALE'] = &$this->SALARY_SCALE;

		// REMARKS
		$this->REMARKS = new DbField('vacancies', 'vacancies', 'x_REMARKS', 'REMARKS', '`REMARKS`', '`REMARKS`', 201, 474, -1, FALSE, '`REMARKS`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`vacancies`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->SALARY_SCALE->DbValue = $row['SALARY SCALE'];
	...
	{
		global $Language;
		if ($pageName == "vacanciesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vacanciesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vacanciesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("vacanciesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vacanciesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "vacanciesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vacanciesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vacanciesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vacanciesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vacanciesdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->SALARY_SCALE->setDbValue($rs->fields('SALARY SCALE'));
	...
	public function renderEditRow()
	{
		...
		// SALARY SCALE
		$this->SALARY_SCALE->EditAttrs["class"] = "form-control";
		$this->SALARY_SCALE->EditCustomAttributes = "";
		if (!$this->SALARY_SCALE->Raw)
			$this->SALARY_SCALE->CurrentValue = HtmlDecode($this->SALARY_SCALE->CurrentValue);
		$this->SALARY_SCALE->EditValue = $this->SALARY_SCALE->CurrentValue;
		$this->SALARY_SCALE->PlaceHolder = RemoveHtml($this->SALARY_SCALE->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->SALARY_SCALE);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->SALARY_SCALE);
