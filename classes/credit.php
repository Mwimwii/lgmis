...
/**
 * Table class for credit
 */
class credit extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'credit';
		$this->TableName = 'credit';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`credit`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// IncomeCode
		$this->IncomeCode = new DbField('credit', 'credit', 'x_IncomeCode', 'IncomeCode', '`IncomeCode`', '`IncomeCode`', 200, 15, -1, FALSE, '`IncomeCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// PayrollPeriod
		$this->PayrollPeriod = new DbField('credit', 'credit', 'x_PayrollPeriod', 'PayrollPeriod', '`PayrollPeriod`', '`PayrollPeriod`', 3, 11, -1, FALSE, '`PayrollPeriod`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// IncomeName
		$this->IncomeName = new DbField('credit', 'credit', 'x_IncomeName', 'IncomeName', '`IncomeName`', '`IncomeName`', 200, 255, -1, FALSE, '`IncomeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// TotalIncomes
		$this->TotalIncomes = new DbField('credit', 'credit', 'x_TotalIncomes', 'TotalIncomes', '`TotalIncomes`', '`TotalIncomes`', 5, 23, -1, FALSE, '`TotalIncomes`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`credit`";
	}
	...
	{
		global $Language;
		if ($pageName == "creditview.php")
			return $Language->phrase("View");
		elseif ($pageName == "creditedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "creditadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("creditview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("creditview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "creditadd.php?" . $this->getUrlParm($parm);
		else
			$url = "creditadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("creditedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("creditadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("creditdelete.php", $this->getUrlParm());
	}
