...
/**
 * Table class for salary_scale
 */
class salary_scale extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'salary_scale';
		$this->TableName = 'salary_scale';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`salary_scale`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// Division
		$this->Division = new DbField('salary_scale', 'salary_scale', 'x_Division', 'Division', '`Division`', '`Division`', 16, 3, -1, FALSE, '`Division`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// SalaryScale
		$this->SalaryScale = new DbField('salary_scale', 'salary_scale', 'x_SalaryScale', 'SalaryScale', '`SalaryScale`', '`SalaryScale`', 200, 10, -1, FALSE, '`SalaryScale`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "salary_notch") {
			$detailUrl = $GLOBALS["salary_notch"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_SalaryScale=" . urlencode($this->SalaryScale->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "salary_scalelist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`salary_scale`";
	}
	...
	{
		global $Language;
		if ($pageName == "salary_scaleview.php")
			return $Language->phrase("View");
		elseif ($pageName == "salary_scaleedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "salary_scaleadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("salary_scaleview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("salary_scaleview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "salary_scaleadd.php?" . $this->getUrlParm($parm);
		else
			$url = "salary_scaleadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("salary_scaleedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("salary_scaleedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("salary_scaleadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("salary_scaleadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("salary_scaledelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->SalaryScale->CurrentValue = $key;
			else
				$this->SalaryScale->OldValue = $key;
