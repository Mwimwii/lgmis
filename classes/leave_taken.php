...
/**
 * Table class for leave_taken
 */
class leave_taken extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'leave_taken';
		$this->TableName = 'leave_taken';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`leave_taken`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('leave_taken', 'leave_taken', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// LeaveTypeCode
		$this->LeaveTypeCode = new DbField('leave_taken', 'leave_taken', 'x_LeaveTypeCode', 'LeaveTypeCode', '`LeaveTypeCode`', '`LeaveTypeCode`', 16, 4, -1, FALSE, '`LeaveTypeCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// StartDate
		$this->StartDate = new DbField('leave_taken', 'leave_taken', 'x_StartDate', 'StartDate', '`StartDate`', CastDateFieldForLike("`StartDate`", 0, "DB"), 133, 10, 0, FALSE, '`StartDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// EndDate
		$this->EndDate = new DbField('leave_taken', 'leave_taken', 'x_EndDate', 'EndDate', '`EndDate`', CastDateFieldForLike("`EndDate`", 0, "DB"), 133, 10, 0, FALSE, '`EndDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Commuted
		$this->Commuted = new DbField('leave_taken', 'leave_taken', 'x_Commuted', 'Commuted', '`Commuted`', '`Commuted`', 16, 1, -1, FALSE, '`Commuted`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Commuted->Sortable = TRUE; // Allow sort
		$this->Commuted->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['Commuted'] = &$this->Commuted;

		// LeaveDays
		$this->LeaveDays = new DbField('leave_taken', 'leave_taken', 'x_LeaveDays', 'LeaveDays', '`LeaveDays`', '`LeaveDays`', 5, 22, -1, FALSE, '`LeaveDays`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Location
		$this->Location = new DbField('leave_taken', 'leave_taken', 'x_Location', 'Location', '`Location`', '`Location`', 200, 255, -1, FALSE, '`Location`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Location->Sortable = TRUE; // Allow sort
		$this->fields['Location'] = &$this->Location;

		// Remarks
		$this->Remarks = new DbField('leave_taken', 'leave_taken', 'x_Remarks', 'Remarks', '`Remarks`', '`Remarks`', 200, 255, -1, FALSE, '`Remarks`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`leave_taken`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->Commuted->DbValue = $row['Commuted'];
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "leave_takenedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "leave_takenadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("leave_takenview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("leave_takenview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "leave_takenadd.php?" . $this->getUrlParm($parm);
		else
			$url = "leave_takenadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("leave_takenedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("leave_takenadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("leave_takendelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->Commuted->setDbValue($rs->fields('Commuted'));
	...
	public function renderEditRow()
	{
		...
		$this->Commuted->EditValue = $this->Commuted->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->Commuted);
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		if ($pend < $pstart) {
		 ...
		 $leavetaken = $rsnew["LeaveDays"];
		 $leavecomm = $rsnew["Commuted"];
		 $leavetype = $rsnew["LeaveTypeCode"];
		 $emp = $rsnew["EmployeeID"];
		 execute("update leave_record set LeaveTaken = LeaveTaken + '" . $leavetaken . "' , LeaveCommuted
		 = LeaveCommuted + '" . $leavecomm . "'	 where EmployeeId = '" . $emp . "'
	...
	/*	 $leavetaken = $rsnew["LeaveDays"];
		 ...
		 $leavetype = $rsnew["LeaveTypeCode"];
		 $emp = $rsnew["EmployeeID"];
		 execute("update leave_record set LeaveTaken = " . $leavetaken . " , LeaveCommuted
		 = LeaveCommuted + " . $leavecomm . "
		 where EmployeeId = " . $emp . " and LeaveTypeCode = " . $leavetype . "); */
	}
	// Row Updating event
	function Row_Updating($rsold, &$rsnew) {
		...
		if ($pend < $pstart) {
		 ...
		 $oldl = $rsold["LeaveDays"];
		 $newl = $rsnew["LeaveDays"];
		 if ($oldl === NULL) {$oldl = 0;}
		 if ($oldl === NULL) {$newl = 0;}
		 $leavetaken = $newl - $oldl;
		 $leavecomm = $rsnew["Commuted"]- $rsold["Commuted"];
		 //die(strval($leavetaken));
		 $leavetype = $rsnew["LeaveTypeCode"];
		 $emp = $rsnew["EmployeeID"];
		 execute("update leave_record set LeaveTaken = '" . $leavetaken . "' , LeaveCommuted
		 = LeaveCommuted + '" . $leavecomm . "'
