...
/**
 * Table class for monthly_journal
 */
class monthly_journal extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'monthly_journal';
		$this->TableName = 'monthly_journal';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`monthly_journal`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// CREDIT
		$this->CREDIT = new DbField('monthly_journal', 'monthly_journal', 'x_CREDIT', 'CREDIT', '`CREDIT`', '`CREDIT`', 200, 6, -1, FALSE, '`CREDIT`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CREDIT->Nullable = FALSE; // NOT NULL field
		$this->CREDIT->Required = TRUE; // Required field
		$this->CREDIT->Sortable = TRUE; // Allow sort
		$this->fields['CREDIT'] = &$this->CREDIT;

		// IncomeName
		$this->IncomeName = new DbField('monthly_journal', 'monthly_journal', 'x_IncomeName', 'IncomeName', '`IncomeName`', '`IncomeName`', 200, 255, -1, FALSE, '`IncomeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// TotalIncomes
		$this->TotalIncomes = new DbField('monthly_journal', 'monthly_journal', 'x_TotalIncomes', 'TotalIncomes', '`TotalIncomes`', '`TotalIncomes`', 201, 417, -1, FALSE, '`TotalIncomes`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->TotalIncomes->Sortable = TRUE; // Allow sort
		$this->fields['TotalIncomes'] = &$this->TotalIncomes;

		// DEBIT
		$this->DEBIT = new DbField('monthly_journal', 'monthly_journal', 'x_DEBIT', 'DEBIT', '`DEBIT`', '`DEBIT`', 201, 65530, -1, FALSE, '`DEBIT`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->DEBIT->Nullable = FALSE; // NOT NULL field
		$this->DEBIT->Required = TRUE; // Required field
		$this->DEBIT->Sortable = TRUE; // Allow sort
		$this->fields['DEBIT'] = &$this->DEBIT;

		// DeductionName
		$this->DeductionName = new DbField('monthly_journal', 'monthly_journal', 'x_DeductionName', 'DeductionName', '`DeductionName`', '`DeductionName`', 201, 65530, -1, FALSE, '`DeductionName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// TotalDeductions
		$this->TotalDeductions = new DbField('monthly_journal', 'monthly_journal', 'x_TotalDeductions', 'TotalDeductions', '`TotalDeductions`', '`TotalDeductions`', 201, 417, -1, FALSE, '`TotalDeductions`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->TotalDeductions->Nullable = FALSE; // NOT NULL field
		$this->TotalDeductions->Required = TRUE; // Required field
		$this->TotalDeductions->Sortable = TRUE; // Allow sort
		$this->fields['TotalDeductions'] = &$this->TotalDeductions;

		// -
		$this->_ = new DbField('monthly_journal', 'monthly_journal', 'x__', '-', '`-`', '`-`', 200, 1, -1, FALSE, '`-`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_->Nullable = FALSE; // NOT NULL field
		$this->_->Required = TRUE; // Required field
		$this->_->Sortable = TRUE; // Allow sort
		$this->fields['-'] = &$this->_;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`monthly_journal`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->TotalIncomes->DbValue = $row['TotalIncomes'];
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "monthly_journallist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "monthly_journalview.php")
			return $Language->phrase("View");
		elseif ($pageName == "monthly_journaledit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "monthly_journaladd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("monthly_journalview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("monthly_journalview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "monthly_journaladd.php?" . $this->getUrlParm($parm);
		else
			$url = "monthly_journaladd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("monthly_journaledit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("monthly_journaladd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("monthly_journaldelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->TotalIncomes->setDbValue($rs->fields('TotalIncomes'));
	...
	public function renderEditRow()
	{
		...
		// TotalIncomes
		$this->TotalIncomes->EditAttrs["class"] = "form-control";
		$this->TotalIncomes->EditCustomAttributes = "";
		$this->TotalIncomes->EditValue = $this->TotalIncomes->CurrentValue;
		$this->TotalIncomes->PlaceHolder = RemoveHtml($this->TotalIncomes->caption());
		...
		// -
		$this->_->EditAttrs["class"] = "form-control";
		$this->_->EditCustomAttributes = "";
		if (!$this->_->Raw)
			$this->_->CurrentValue = HtmlDecode($this->_->CurrentValue);
		$this->_->EditValue = $this->_->CurrentValue;
		$this->_->PlaceHolder = RemoveHtml($this->_->caption());
