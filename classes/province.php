...
class province extends DbTable
{
	...
	public function __construct()
	{
		...
		// ProvinceCode
		$this->ProvinceCode = new DbField('province', 'province', 'x_ProvinceCode', 'ProvinceCode', '`ProvinceCode`', '`ProvinceCode`', 17, 3, -1, FALSE, '`ProvinceCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ProvinceName
		$this->ProvinceName = new DbField('province', 'province', 'x_ProvinceName', 'ProvinceName', '`ProvinceName`', '`ProvinceName`', 200, 40, -1, FALSE, '`ProvinceName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// LastUserID
		$this->LastUserID = new DbField('province', 'province', 'x_LastUserID', 'LastUserID', '`LastUserID`', '`LastUserID`', 200, 40, -1, FALSE, '`LastUserID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->LastUserID->Sortable = TRUE; // Allow sort
		$this->fields['LastUserID'] = &$this->LastUserID;

		// LastUpdated
		$this->LastUpdated = new DbField('province', 'province', 'x_LastUpdated', 'LastUpdated', '`LastUpdated`', CastDateFieldForLike("`LastUpdated`", 0, "DB"), 135, 19, 0, FALSE, '`LastUpdated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Created
		$this->Created = new DbField('province', 'province', 'x_Created', 'Created', '`Created`', CastDateFieldForLike("`Created`", 0, "DB"), 133, 10, 0, FALSE, '`Created`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Comment
		$this->Comment = new DbField('province', 'province', 'x_Comment', 'Comment', '`Comment`', '`Comment`', 200, 255, -1, FALSE, '`Comment`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "local_authority") {
			$detailUrl = $GLOBALS["local_authority"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_ProvinceCode=" . urlencode($this->ProvinceCode->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "provincelist.php";
		return $detailUrl;
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("provinceview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("provinceview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "provinceadd.php?" . $this->getUrlParm($parm);
		else
			$url = "provinceadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("provinceedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("provinceedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("provinceadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("provinceadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("provincedelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->ProvinceCode->CurrentValue = $key;
			else
				$this->ProvinceCode->OldValue = $key;
	...
	function Recordset_Selecting(&$filter) {
		...
		$dept = executeRow("select count(security_matrix.ProvinceCode)as kountdept 
		...
		AddFilter($filter,"`ProvinceCode`  in   (select security_matrix.ProvinceCode
		from security_matrix, musers                            
		where security_matrix.usercode = musers.usercode 
		and musers.username = '" . $username .  
		"')  ");  }
		elseif ($levelid >= 0)  {AddFilter($filter,"`ProvinceCode`  
		in   (SELECT DISTINCT Province.`provinceCode`
		FROM security_matrix, province, musers                            
		WHERE security_matrix.usercode = musers.usercode 
		AND security_matrix.ProvinceCode = Province.ProvinceCode 
		AND musers.username = '" . $username .  
		"')  "); 
		}  
	}
