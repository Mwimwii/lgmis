...
/**
 * Table class for accountgroup
 */
class accountgroup extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'accountgroup';
		$this->TableName = 'accountgroup';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`accountgroup`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// AccountGroupCode
		$this->AccountGroupCode = new DbField('accountgroup', 'accountgroup', 'x_AccountGroupCode', 'AccountGroupCode', '`AccountGroupCode`', '`AccountGroupCode`', 3, 11, -1, FALSE, '`AccountGroupCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// AccountGroupName
		$this->AccountGroupName = new DbField('accountgroup', 'accountgroup', 'x_AccountGroupName', 'AccountGroupName', '`AccountGroupName`', '`AccountGroupName`', 200, 255, -1, FALSE, '`AccountGroupName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->AccountGroupName->Sortable = TRUE; // Allow sort
		$this->fields['AccountGroupName'] = &$this->AccountGroupName;

		// AccountType
		$this->AccountType = new DbField('accountgroup', 'accountgroup', 'x_AccountType', 'AccountType', '`AccountType`', '`AccountType`', 3, 11, -1, FALSE, '`AccountType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "account_type") {
			if ($this->AccountType->getSessionValue() != "")
				$masterFilter .= "`AccountTypeCode`=" . QuotedValue($this->AccountType->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	// Master filter
	public function sqlMasterFilter_account_type()
	{
		return "`AccountTypeCode`=@AccountTypeCode@";
	}

	// Detail filter
	public function sqlDetailFilter_account_type()
	{
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "account_sub_group") {
			$detailUrl = $GLOBALS["account_sub_group"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
		...
		if ($detailUrl == "")
			$detailUrl = "accountgrouplist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`accountgroup`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "accountgroupedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "accountgroupadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("accountgroupview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("accountgroupview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "accountgroupadd.php?" . $this->getUrlParm($parm);
		else
			$url = "accountgroupadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("accountgroupedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("accountgroupadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("accountgroupdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "account_type" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_AccountTypeCode=" . urlencode($this->AccountType->CurrentValue);
		}
		return $url;
	}
