...
/**
 * Table class for currently_employed_staff
 */
class currently_employed_staff extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'currently_employed_staff';
		$this->TableName = 'currently_employed_staff';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`currently_employed_staff`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('currently_employed_staff', 'currently_employed_staff', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// FirstName
		$this->FirstName = new DbField('currently_employed_staff', 'currently_employed_staff', 'x_FirstName', 'FirstName', '`FirstName`', '`FirstName`', 200, 100, -1, FALSE, '`FirstName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// MiddleName
		$this->MiddleName = new DbField('currently_employed_staff', 'currently_employed_staff', 'x_MiddleName', 'MiddleName', '`MiddleName`', '`MiddleName`', 200, 100, -1, FALSE, '`MiddleName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->MiddleName->Sortable = TRUE; // Allow sort
		$this->fields['MiddleName'] = &$this->MiddleName;

		// Surname
		$this->Surname = new DbField('currently_employed_staff', 'currently_employed_staff', 'x_Surname', 'Surname', '`Surname`', '`Surname`', 200, 100, -1, FALSE, '`Surname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`currently_employed_staff`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "currently_employed_stafflist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "currently_employed_staffview.php")
			return $Language->phrase("View");
		elseif ($pageName == "currently_employed_staffedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "currently_employed_staffadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("currently_employed_staffview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("currently_employed_staffview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "currently_employed_staffadd.php?" . $this->getUrlParm($parm);
		else
			$url = "currently_employed_staffadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("currently_employed_staffedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("currently_employed_staffadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("currently_employed_staffdelete.php", $this->getUrlParm());
	}
