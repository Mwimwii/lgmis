...
/**
 * Table class for leave_accrued_trans
 */
class leave_accrued_trans extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'leave_accrued_trans';
		$this->TableName = 'leave_accrued_trans';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`leave_accrued_trans`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// Year
		$this->Year = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_Year', 'Year', '`Year`', '`Year`', 18, 4, -1, FALSE, '`Year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// RunMonth
		$this->RunMonth = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_RunMonth', 'RunMonth', '`RunMonth`', '`RunMonth`', 16, 2, -1, FALSE, '`RunMonth`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// EmployeeID
		$this->EmployeeID = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// LeaveTypeCode
		$this->LeaveTypeCode = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_LeaveTypeCode', 'LeaveTypeCode', '`LeaveTypeCode`', '`LeaveTypeCode`', 16, 4, -1, FALSE, '`LeaveTypeCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// LeaveAccrued
		$this->LeaveAccrued = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_LeaveAccrued', 'LeaveAccrued', '`LeaveAccrued`', '`LeaveAccrued`', 5, 22, -1, FALSE, '`LeaveAccrued`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// LastAccrualDate
		$this->LastAccrualDate = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_LastAccrualDate', 'LastAccrualDate', '`LastAccrualDate`', CastDateFieldForLike("`LastAccrualDate`", 0, "DB"), 133, 10, 0, FALSE, '`LastAccrualDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// LeaveLost
		$this->LeaveLost = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_LeaveLost', 'LeaveLost', '`LeaveLost`', '`LeaveLost`', 5, 22, -1, FALSE, '`LeaveLost`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->LeaveLost->Sortable = TRUE; // Allow sort
		$this->LeaveLost->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['LeaveLost'] = &$this->LeaveLost;

		// LACode
		$this->LACode = new DbField('leave_accrued_trans', 'leave_accrued_trans', 'x_LACode', 'LACode', '`LACode`', '`LACode`', 200, 10, -1, FALSE, '`LACode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`leave_accrued_trans`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->LeaveLost->DbValue = $row['LeaveLost'];
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "leave_accrued_translist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "leave_accrued_transedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "leave_accrued_transadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("leave_accrued_transview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("leave_accrued_transview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "leave_accrued_transadd.php?" . $this->getUrlParm($parm);
		else
			$url = "leave_accrued_transadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("leave_accrued_transedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("leave_accrued_transadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("leave_accrued_transdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->LeaveLost->setDbValue($rs->fields('LeaveLost'));
	...
	public function renderEditRow()
	{
		...
		$this->RunMonth->EditValue = $this->RunMonth->CurrentValue;
		...
		$this->LeaveLost->EditValue = $this->LeaveLost->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->LeaveLost);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->LeaveLost);
