...
/**
 * Table class for lasf
 */
class lasf extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// EmployeeID
		$this->EmployeeID = new DbField('lasf', 'lasf', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// FirstName
		$this->FirstName = new DbField('lasf', 'lasf', 'x_FirstName', 'FirstName', '`FirstName`', '`FirstName`', 200, 100, -1, FALSE, '`FirstName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Surname
		$this->Surname = new DbField('lasf', 'lasf', 'x_Surname', 'Surname', '`Surname`', '`Surname`', 200, 100, -1, FALSE, '`Surname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// NRC
		$this->NRC = new DbField('lasf', 'lasf', 'x_NRC', 'NRC', '`NRC`', '`NRC`', 200, 13, -1, FALSE, '`NRC`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateOfBirth
		$this->DateOfBirth = new DbField('lasf', 'lasf', 'x_DateOfBirth', 'DateOfBirth', '`DateOfBirth`', CastDateFieldForLike("`DateOfBirth`", 0, "DB"), 133, 10, 0, FALSE, '`DateOfBirth`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Sex
		$this->Sex = new DbField('lasf', 'lasf', 'x_Sex', 'Sex', '`Sex`', '`Sex`', 200, 6, -1, FALSE, '`Sex`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// BasicSalary
		$this->BasicSalary = new DbField('lasf', 'lasf', 'x_BasicSalary', 'BasicSalary', '`BasicSalary`', '`BasicSalary`', 5, 23, -1, FALSE, '`BasicSalary`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DeductionName
		$this->DeductionName = new DbField('lasf', 'lasf', 'x_DeductionName', 'DeductionName', '`DeductionName`', '`DeductionName`', 200, 255, -1, FALSE, '`DeductionName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->Employee_Contribution = new DbField('lasf', 'lasf', 'x_Employee_Contribution', 'Employee Contribution', '`Employee Contribution`', '`Employee Contribution`', 5, 22, -1, FALSE, '`Employee Contribution`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->Employer_Contribution = new DbField('lasf', 'lasf', 'x_Employer_Contribution', 'Employer Contribution', '`Employer Contribution`', '`Employer Contribution`', 5, 23, -1, FALSE, '`Employer Contribution`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Total Contribution
		$this->Total_Contribution = new DbField('lasf', 'lasf', 'x_Total_Contribution', 'Total Contribution', '`Total Contribution`', '`Total Contribution`', 5, 23, -1, FALSE, '`Total Contribution`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Total_Contribution->Sortable = TRUE; // Allow sort
		$this->Total_Contribution->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['Total Contribution'] = &$this->Total_Contribution;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`lasf`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->Total_Contribution->DbValue = $row['Total Contribution'];
	}
	...
	{
		global $Language;
		if ($pageName == "lasfview.php")
			return $Language->phrase("View");
		elseif ($pageName == "lasfedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "lasfadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("lasfview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("lasfview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "lasfadd.php?" . $this->getUrlParm($parm);
		else
			$url = "lasfadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("lasfedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("lasfadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("lasfdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->Total_Contribution->setDbValue($rs->fields('Total Contribution'));
	}
	...
	public function renderEditRow()
	{
		...
		// Total Contribution
		$this->Total_Contribution->EditAttrs["class"] = "form-control";
		$this->Total_Contribution->EditCustomAttributes = "";
		$this->Total_Contribution->EditValue = $this->Total_Contribution->CurrentValue;
		$this->Total_Contribution->PlaceHolder = RemoveHtml($this->Total_Contribution->caption());
		if (strval($this->Total_Contribution->EditValue) != "" && is_numeric($this->Total_Contribution->EditValue))
			$this->Total_Contribution->EditValue = FormatNumber($this->Total_Contribution->EditValue, -2, -2, -2, -2);
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->Total_Contribution);
				} else {
					$doc->exportCaption($this->EmployeeID);
					$doc->exportCaption($this->FirstName);
					$doc->exportCaption($this->Surname);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->Total_Contribution);
					} else {
						$doc->exportField($this->EmployeeID);
						$doc->exportField($this->FirstName);
						$doc->exportField($this->Surname);
