...
/**
 * Table class for la_program
 */
class la_program extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'la_program';
		$this->TableName = 'la_program';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`la_program`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// ProgramCode
		$this->ProgramCode = new DbField('la_program', 'la_program', 'x_ProgramCode', 'ProgramCode', '`ProgramCode`', '`ProgramCode`', 3, 11, -1, FALSE, '`ProgramCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ProgramName
		$this->ProgramName = new DbField('la_program', 'la_program', 'x_ProgramName', 'ProgramName', '`ProgramName`', '`ProgramName`', 200, 255, -1, FALSE, '`ProgramName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ProgramPurpose
		$this->ProgramPurpose = new DbField('la_program', 'la_program', 'x_ProgramPurpose', 'ProgramPurpose', '`ProgramPurpose`', '`ProgramPurpose`', 200, 255, -1, FALSE, '`ProgramPurpose`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "la_sub_program") {
			$detailUrl = $GLOBALS["la_sub_program"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_ProgramCode=" . urlencode($this->ProgramCode->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "la_programlist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`la_program`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ProgramCode', $rs))
				AddFilter($where, QuotedName('ProgramCode', $this->Dbid) . '=' . QuotedValue($rs['ProgramCode'], $this->ProgramCode->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "la_programedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "la_programadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("la_programview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("la_programview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "la_programadd.php?" . $this->getUrlParm($parm);
		else
			$url = "la_programadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("la_programedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("la_programedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("la_programadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("la_programadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("la_programdelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->ProgramCode->CurrentValue = $key;
			else
				$this->ProgramCode->OldValue = $key;
