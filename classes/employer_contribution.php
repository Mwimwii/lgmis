...
/**
 * Table class for employer_contribution
 */
class employer_contribution extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'employer_contribution';
		$this->TableName = 'employer_contribution';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`employer_contribution`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// PayrollPeriod
		$this->PayrollPeriod = new DbField('employer_contribution', 'employer_contribution', 'x_PayrollPeriod', 'PayrollPeriod', '`PayrollPeriod`', '`PayrollPeriod`', 3, 11, -1, FALSE, '`PayrollPeriod`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DeductionCode
		$this->DeductionCode = new DbField('employer_contribution', 'employer_contribution', 'x_DeductionCode', 'DeductionCode', '`DeductionCode`', '`DeductionCode`', 200, 15, -1, FALSE, '`DeductionCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DeductionName
		$this->DeductionName = new DbField('employer_contribution', 'employer_contribution', 'x_DeductionName', 'DeductionName', '`DeductionName`', '`DeductionName`', 201, 277, -1, FALSE, '`DeductionName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		...
		// TotalDeductions
		$this->TotalDeductions = new DbField('employer_contribution', 'employer_contribution', 'x_TotalDeductions', 'TotalDeductions', '`TotalDeductions`', '`TotalDeductions`', 5, 23, -1, FALSE, '`TotalDeductions`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`employer_contribution`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "employer_contributionlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "employer_contributionview.php")
			return $Language->phrase("View");
		elseif ($pageName == "employer_contributionedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "employer_contributionadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("employer_contributionview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("employer_contributionview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "employer_contributionadd.php?" . $this->getUrlParm($parm);
		else
			$url = "employer_contributionadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("employer_contributionedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("employer_contributionadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("employer_contributiondelete.php", $this->getUrlParm());
	}
