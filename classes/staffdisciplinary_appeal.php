...
/**
 * Table class for staffdisciplinary_appeal
 */
class staffdisciplinary_appeal extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'staffdisciplinary_appeal';
		$this->TableName = 'staffdisciplinary_appeal';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`staffdisciplinary_appeal`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// EmployeeID
		$this->EmployeeID = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_EmployeeID', 'EmployeeID', '`EmployeeID`', '`EmployeeID`', 3, 11, -1, FALSE, '`EmployeeID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// CaseNo
		$this->CaseNo = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_CaseNo', 'CaseNo', '`CaseNo`', '`CaseNo`', 3, 11, -1, FALSE, '`CaseNo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->CaseNo->IsPrimaryKey = TRUE; // Primary key field
		$this->CaseNo->IsForeignKey = TRUE; // Foreign key field
		$this->CaseNo->Nullable = FALSE; // NOT NULL field
		...
		// OffenseCode
		$this->OffenseCode = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_OffenseCode', 'OffenseCode', '`OffenseCode`', '`OffenseCode`', 3, 11, -1, FALSE, '`OffenseCode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// AppealNo
		$this->AppealNo = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_AppealNo', 'AppealNo', '`AppealNo`', '`AppealNo`', 3, 11, -1, FALSE, '`AppealNo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->AppealNo->IsAutoIncrement = TRUE; // Autoincrement field
		$this->AppealNo->IsPrimaryKey = TRUE; // Primary key field
		$this->AppealNo->Sortable = TRUE; // Allow sort
		$this->AppealNo->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['AppealNo'] = &$this->AppealNo;

		// DateOfAppealLetter
		$this->DateOfAppealLetter = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_DateOfAppealLetter', 'DateOfAppealLetter', '`DateOfAppealLetter`', CastDateFieldForLike("`DateOfAppealLetter`", 0, "DB"), 133, 10, 0, FALSE, '`DateOfAppealLetter`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateAppealReceived
		$this->DateAppealReceived = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_DateAppealReceived', 'DateAppealReceived', '`DateAppealReceived`', CastDateFieldForLike("`DateAppealReceived`", 0, "DB"), 133, 10, 0, FALSE, '`DateAppealReceived`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateConcluded
		$this->DateConcluded = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_DateConcluded', 'DateConcluded', '`DateConcluded`', CastDateFieldForLike("`DateConcluded`", 0, "DB"), 133, 10, 0, FALSE, '`DateConcluded`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// AppealStatus
		$this->AppealStatus = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_AppealStatus', 'AppealStatus', '`AppealStatus`', '`AppealStatus`', 3, 11, -1, FALSE, '`AppealStatus`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// LastUpdate
		$this->LastUpdate = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_LastUpdate', 'LastUpdate', '`LastUpdate`', CastDateFieldForLike("`LastUpdate`", 0, "DB"), 135, 19, 0, FALSE, '`LastUpdate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// AppealNotes
		$this->AppealNotes = new DbField('staffdisciplinary_appeal', 'staffdisciplinary_appeal', 'x_AppealNotes', 'AppealNotes', '`AppealNotes`', '`AppealNotes`', 201, 16777215, -1, FALSE, '`AppealNotes`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "staffdisciplinary_case") {
			...
			if ($this->CaseNo->getSessionValue() != "")
				$masterFilter .= " AND `CaseNo`=" . QuotedValue($this->CaseNo->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
			if ($this->OffenseCode->getSessionValue() != "")
				$masterFilter .= " AND `OffenseCode`=" . QuotedValue($this->OffenseCode->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "staffdisciplinary_case") {
			if ($this->EmployeeID->getSessionValue() != "")
				...
				return "";
			if ($this->CaseNo->getSessionValue() != "")
				$detailFilter .= " AND `CaseNo`=" . QuotedValue($this->CaseNo->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
			if ($this->OffenseCode->getSessionValue() != "")
				$detailFilter .= " AND `OffenseCode`=" . QuotedValue($this->OffenseCode->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "staff") {
	...
	// Master filter
	public function sqlMasterFilter_staffdisciplinary_case()
	{
		return "`EmployeeID`=@EmployeeID@ AND `CaseNo`=@CaseNo@ AND `OffenseCode`=@OffenseCode@";
	}

	// Detail filter
	public function sqlDetailFilter_staffdisciplinary_case()
	{
		return "`EmployeeID`=@EmployeeID@ AND `CaseNo`=@CaseNo@ AND `OffenseCode`=@OffenseCode@";
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`staffdisciplinary_appeal`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->AppealNo->setDbValue($conn->insert_ID());
			$rs['AppealNo'] = $this->AppealNo->DbValue;
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			...
			if (array_key_exists('AppealNo', $rs))
				AddFilter($where, QuotedName('AppealNo', $this->Dbid) . '=' . QuotedValue($rs['AppealNo'], $this->AppealNo->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->AppealNo->DbValue = $row['AppealNo'];
	...
	{
		return "`EmployeeID` = @EmployeeID@ AND `CaseNo` = @CaseNo@ AND `OffenseCode` = @OffenseCode@ AND `AppealNo` = @AppealNo@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('AppealNo', $row) ? $row['AppealNo'] : NULL;
		else
			$val = $this->AppealNo->OldValue !== NULL ? $this->AppealNo->OldValue : $this->AppealNo->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@AppealNo@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "staffdisciplinary_appeallist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "staffdisciplinary_appealedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "staffdisciplinary_appealadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("staffdisciplinary_appealview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("staffdisciplinary_appealview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "staffdisciplinary_appealadd.php?" . $this->getUrlParm($parm);
		else
			$url = "staffdisciplinary_appealadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("staffdisciplinary_appealedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("staffdisciplinary_appealadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("staffdisciplinary_appealdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "staffdisciplinary_case" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_EmployeeID=" . urlencode($this->EmployeeID->CurrentValue);
			$url .= "&fk_CaseNo=" . urlencode($this->CaseNo->CurrentValue);
			$url .= "&fk_OffenseCode=" . urlencode($this->OffenseCode->CurrentValue);
		}
	...
	public function keyToJson($htmlEncode = FALSE)
	{
		...
		$json .= ",AppealNo:" . JsonEncode($this->AppealNo->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->AppealNo->CurrentValue != NULL) {
			$url .= "&AppealNo=" . urlencode($this->AppealNo->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		return $url;
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			elseif (IsApi() && Route(4) !== NULL)
				...
				$arKeys = NULL; // Do not setup
			if (Param("AppealNo") !== NULL)
				$arKey[] = Param("AppealNo");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->AppealNo->CurrentValue = $key[3];
			else
				$this->AppealNo->OldValue = $key[3];
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->AppealNo->setDbValue($rs->fields('AppealNo'));
	...
	public function renderListRow()
	{
		...
		// AppealNo
		$this->AppealNo->LinkCustomAttributes = "";
		$this->AppealNo->HrefValue = "";
		$this->AppealNo->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		$this->CaseNo->PlaceHolder = RemoveHtml($this->CaseNo->caption());
		...
		// AppealNo
		$this->AppealNo->EditAttrs["class"] = "form-control";
		$this->AppealNo->EditCustomAttributes = "";
		$this->AppealNo->EditValue = $this->AppealNo->CurrentValue;
		$this->AppealNo->ViewCustomAttributes = "";
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->AppealNo);
	...
	{
		$table = 'staffdisciplinary_appeal';
		$usr = CurrentUserName();
